...
const Couseexercies1 = ({ route }) => {
    const { courseId } = route.params || {};  // ตรวจสอบ route.params
    ...
    useEffect(() => {
      if (courseId) {  // ตรวจสอบว่ามี courseId ก่อนเรียก API
        fetchexercises();
      } else {
        console.error('ไม่มี courseId ที่ถูกส่งมา');
        setLoading(false);
      }
    }, [courseId]);
    ...
    const fetchexercises = async () => {
      try {
        const response = await fetch(
          `http://192.168.1.100:1337/api/add-courses/${courseId}?populate=image,all_exercises.animation,all_exercises.muscle`
        );
