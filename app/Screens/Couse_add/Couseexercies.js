...
const Couseexercies = ({ route }) => {
  const { item, items, currentIndex, courseId, courseName } = route.params;  // Get courseId from navigation
  ...
  useEffect(() => {
    fetchexercises();
  }, [item, items, currentIndex, courseId, courseName]);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate('Couse_startc', { item: exercises[0], items: exercises, currentIndex: 0, isRest: false, courseId, courseName })
        }
      >
      ...
      <FlatList
        data={exercises}
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.exerciseItem} onPress={() => navigation.navigate('Couse_des', { item, items: exercises, courseId, courseName })}>
