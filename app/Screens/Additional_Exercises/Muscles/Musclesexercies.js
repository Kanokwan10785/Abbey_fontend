  ...
  const fetchexercises = async () => {
    try {
      ...
      const exerciseData = data.data?.attributes?.all_exercises?.data.map((exercise) => {
        const imageUrl = exercise.attributes.muscle?.data?.[0]?.attributes?.url || null;
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={exercises}
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.exerciseItem} onPress={() => navigation.navigate('Muscles_des', { item, items: exercises, musclesId, musclesName })}>
