...
const Musclesexercies1 = ({ route }) => {
    const { musclesId } = route.params || {};  // ตรวจสอบ route.params
    ...
    console.log('musclesId in couse Musles1',musclesId)

    useEffect(() => {
      if (musclesId) {  // ตรวจสอบว่ามี musclesId ก่อนเรียก API
        fetchexercises();
      } else {
        console.error('ไม่มี musclesId ที่ถูกส่งมา');
        setLoading(false);
      }
    }, [musclesId]);
    ...
    const fetchexercises = async () => {
      try {
        const response = await fetch(
          `http://192.168.1.125:1337/api/addexercises/${musclesId}?populate=image,all_exercises.animation,all_exercises.muscle`
        );
    ...
    return (
      <View style={styles.container}>
        ...
        <FlatList
          data={exercises}
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.exerciseItem} onPress={() => navigation.navigate('Muscles_des', { item, items: exercises })}>
...
export default Musclesexercies1;
